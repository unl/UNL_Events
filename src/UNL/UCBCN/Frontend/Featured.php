...
class Featured extends EventListing implements RoutableInterface
{
	...
	function getSQL()
	{
		...
		$pinnedResults = $this->getPinnedEvents();
		...
		$featuredResults = $this->getFeaturedEvents();
	...
	private function getFeaturedEvents() {
		$timestamp = $this->getDateTime()->getTimestamp();
		$sql = 'SELECT event.id as eventID, e.id as eventDatetimeID, recurringdate.id as recurringDateID
                FROM eventdatetime as e
                INNER JOIN event ON e.event_id = event.id
                INNER JOIN calendar_has_event ON calendar_has_event.event_id = event.id
                LEFT JOIN recurringdate ON (recurringdate.event_datetime_id = e.id AND recurringdate.unlinked = 0)
                WHERE
                    calendar_has_event.calendar_id = ' . (int)$this->calendar->id . '
                    AND calendar_has_event.featured = 1
                    AND (
                         calendar_has_event.status =\'posted\'
                         OR calendar_has_event.status =\'archived\'
                    )
	...
	private function getPinnedEvents() {
		$timestamp = $this->getDateTime()->getTimestamp();
		$sql = 'SELECT event.id as eventID, e.id as eventDatetimeID, recurringdate.id as recurringDateID
                FROM eventdatetime as e
                INNER JOIN event ON e.event_id = event.id
                ...
                WHERE
                    calendar_has_event.calendar_id = ' . (int)$this->calendar->id . '
                    AND calendar_has_event.pinned = 1
                    AND (
                         calendar_has_event.status =\'posted\'
                         OR calendar_has_event.status =\'archived\'
                    )
		...
		if (is_numeric($this->options['pinned_limit']) && $this->options['pinned_limit'] >= 1) {
			$sql .= ' LIMIT ' . (int)$this->options['pinned_limit'];
		}
