...
class AddUser extends PostHandler
{
    ...
    public function handlePost(array $get, array $post, array $files)
    {
        ...
        if (array_key_exists('user_uid', $this->options)) {
            ...
            $this->flashNotice('success', 'User Permissions Updated', 'User "' . $this->user->uid . '"\'s permissions have been updated.');
        } else {
            # we are adding a new user
            $this->user = $this->addUser($post);
            $this->flashNotice('success', 'User Added', 'User "' . $this->user->uid . '" has been added to the calendar with the permissions you have specified.');
        }
