<?php
    ...
    // We need todo this since the event_datetime gets edited when post is being handled
    ...
    $start_date   = $post['start_date']        ?? ( !empty($datetime_start_time) ? date('m/d/Y', $datetime_start_time) : ''   );
    ...
    $end_date   = $post['end_date']        ?? ( !empty($datetime_end_time) ? date('m/d/Y', $datetime_end_time) : ''   );
        ...
        <div class="dcf-form-group">
            ...
            <select id="timezone" name="timezone" aria-label="Time Zone">
                <?php foreach (UNL\UCBCN::getTimezoneOptions() as $tzName => $tzValue) : ?>
                    <option <?php if ($timezone == $tzValue) echo SELECTED_INPUT; ?> value="<?php echo $tzValue ?>"><?php echo $tzName ?></option>
                <?php endforeach; ?>
            </select>
        </div>
            ...
            <div class="dcf-d-flex dcf-flex-wrap dcf-ai-center dcf-col-gap-4">
                ...
                <div class="dcf-form-group dcf-d-flex dcf-ai-center dcf-flex-grow-1">
                    ...
                    <select class="dcf-flex-grow-1" id="start-time-hour" name="start_time_hour" aria-label="Start Time Hour">
                        ...
                        <?php for ($i = 1; $i <= 12; $i++) { ?>
                            <option <?php if ($start_hour == $i) echo SELECTED_INPUT; ?> value="<?php echo $i ?>"><?php echo $i ?></option>
                        <?php } ?>
                    </select>
                    ...
                    <select class="dcf-flex-grow-1" id="start-time-minute" name="start_time_minute" aria-label="Start Time Minute">
                        ...
                        <?php for ($i = 0; $i < 60; $i+=5): ?>
                            <option <?php if ($start_minute == $i) echo SELECTED_INPUT; ?> value="<?php echo $i; ?>"><?php echo str_pad($i, 2, '0', STR_PAD_LEFT); ?></option>
                        <?php endfor; ?>
                    </select>
            ...
            <div class="dcf-d-flex dcf-flex-wrap dcf-ai-center dcf-col-gap-4">
                ...
                <div class="dcf-form-group dcf-d-flex dcf-ai-center dcf-flex-grow-1">
                    ...
                    <select class="dcf-flex-grow-1" id="end-time-hour" name="end_time_hour" aria-label="End Time Hour">
                        <option value="">Hour</option>
                        <?php for ($i = 1; $i <= 12; $i++) { ?>
                            <option <?php if ($end_hour == $i) echo SELECTED_INPUT; ?> value="<?php echo $i ?>"><?php echo $i ?></option>
                        <?php } ?>
                    </select>
                    <span class="dcf-pr-1 dcf-pl-1">:</span>
        ...
        <?php if (!$datetime_recurring_check) : ?>
            <div class="section-container">
                <div class="dcf-input-checkbox">
                    <input id="recurring" name="recurring" type="checkbox" <?php if ($is_recurring) { echo CHECKED_INPUT; } ?>>
                ...
                <fieldset class="recurring-container date-time-select">
                    ...
                    <div class="dcf-form-group">
                        ...
                        <select id="recurring-type" name="recurring_type">
                            <option value="daily" <?php if($recurring_type == "daily") { echo SELECTED_INPUT; } ?>>Daily</option>
                            <option value="weekly" <?php if($recurring_type == "weekly") { echo SELECTED_INPUT; } ?>>Weekly</option>
                            <option value="biweekly" <?php if($recurring_type == "biweekly") { echo SELECTED_INPUT; } ?>>Biweekly</option>
                            <optgroup label="Monthly" id="monthly-group">
                            </optgroup>
                            <option value="annually" <?php if($recurring_type == "annually") { echo SELECTED_INPUT; } ?>>Yearly</option>
                        </select>
                    </div>
                    <div class="dcf-form-group dcf-datepicker">
                        <label for="recurs-until-date">until </label>
                        <input id="recurs-until-date" name="recurs_until_date" type="text" value="<?php echo $recurs_until_date; ?>" aria-label="until this date in the format of mm/dd/yyyy" autocomplete="off" />
                    </div>
                    </div>
                </fieldset>
            </div>
...
<section id="physical_location_section" class="dcf-mb-8 dcf-ml-5">
    ...
    <input type="hidden" id="physical_location_check" name="physical_location_check" value="<?php echo $datetime_physical_location_check; ?>">
    ...
    <template id="physical_location_template">
        <div class="dcf-form-group">
            ...
            <select id="location" name="location">
                ...
                <option <?php if (empty($location) || $location == 'new') echo SELECTED_INPUT ?>value="new">-- New Location --</option>
        ...
        <fieldset class="dcf-mt-6" id="new-location-fields" style="display: none;">
            ...
            <div class="dcf-d-grid dcf-grid-full dcf-grid-halves@md dcf-col-gap-5">
                <div class="dcf-form-group events-col-full-width">
                    ...
                    <input id="location-name" class="dcf-w-100%" name="new_location[name]" type="text" value="<?php echo isset($post['new_location']['name']) ? $post['new_location']['name']: ''; ?>">
                </div>

                <div class="dcf-form-group">
                    <label for="location-address-1">Address <small class="dcf-required">Required</small></label>
                    <input id="location-address-1" class="dcf-w-100%" name="new_location[streetaddress1]" type="text" value="<?php echo isset($post['new_location']['streetaddress1']) ? $post['new_location']['streetaddress1']: ''; ?>">
                </div>
                <div class="dcf-form-group">
                    <label for="location-address-2">Address 2</label>
                    <input id="location-address-2" class="dcf-w-100%" name="new_location[streetaddress2]" type="text" value="<?php echo isset($post['new_location']['streetaddress2']) ? $post['new_location']['streetaddress2']: ''; ?>">
                </div>

                <div class="dcf-form-group">
                    <label for="location-city">City <small class="dcf-required">Required</small></label>
                    <input id="location-city" class="dcf-w-100%" name="new_location[city]" type="text" value="<?php echo isset($post['new_location']['city']) ? $post['new_location']['city']: ''; ?>">
                </div>
                <div class="dcf-form-group">
                    ...
                    <select name="new_location[state]" id="location-state">
                        <?php foreach($states as $abbr => $state): ?>
                            <option value="<?php echo $abbr; ?>"
                                <?php if (isset($post['new_location']['state']) && $post['new_location']['state'] == $abbr): ?>
                                    selected="selected"
                ...
                <div class="dcf-form-group">
                    ...
                    <input id="location-map-url" class="dcf-w-100%" name="new_location[mapurl]" type="text" value="<?php echo isset($post['new_location']['mapurl']) ? $post['new_location']['mapurl']: ''; ?>">
                </div>
                <div class="dcf-form-group">
                    <label for="location-webpage">Web Page</label>
                    <input id="location-webpage" class="dcf-w-100%" name="new_location[webpageurl]" type="text" value="<?php echo isset($post['new_location']['webpageurl']) ? $post['new_location']['webpageurl']: ''; ?>">
                </div>

                <div class="dcf-form-group">
                    <label for="location-hours">Hours</label>
                    <input id="location-hours" class="dcf-w-100%" name="new_location[hours]" ype="text" value="<?php echo isset($post['new_location']['hours']) ? $post['new_location']['hours']: ''; ?>">
                </div>
                <div class="dcf-form-group">
                    <label for="location-phone">Phone</label>
                    <input id="location-phone" class="dcf-w-100%" name="new_location[phone]" type="text" value="<?php echo isset($post['new_location']['phone']) ? $post['new_location']['phone']: ''; ?>">
                </div>
                ...
                <div class="dcf-form-group events-col-full-width">
                    ...
                    <input id="location-room" name="new_location[room]" type="text" value="<?php echo isset($post['new_location']['room']) ? $post['new_location']['room']: ''; ?>">
                </div>

                <div class="dcf-form-group events-col-full-width">
                    <label for="location-directions">Location Default - Directions</label>
                    <textarea id="location-directions" name="new_location[directions]"><?php echo isset($post['new_location']['directions']) ? $post['new_location']['directions']: ''; ?></textarea>
                </div>

                <div class="dcf-form-group events-col-full-width">
                    <label for="location-additional-public-info">Location Default - Additional Public Info</label>
                    <textarea id="location-additional-public-info" name="new_location[additionalpublicinfo]"><?php echo isset($post['new_location']['additionalpublicinfo']) ? $post['new_location']['additionalpublicinfo']: ''; ?></textarea>
                </div>
...
<section id="virtual_location_section" class="dcf-mb-8 dcf-ml-5">
    ...
    <input type="hidden" id="virtual_location_check" name="virtual_location_check" value="<?php echo $datetime_virtual_location_check; ?>">
    ...
    <template id="virtual_location_template">
        <div class="dcf-form-group">
            ...
            <select id="v-location" name="v_location">
                ...
                <option <?php if (empty($v_location) || $v_location == 'new') { echo SELECTED_INPUT; } ?>value="new">-- New Location --</option>
                ...
                <?php else: ?>
                    <optgroup label="Your saved locations">
                        <?php foreach ($user_webcasts as $webcast): ?>
                            <option <?php if ($v_location == $webcast->id) echo SELECTED_INPUT ?> value="<?php echo $webcast->id ?>"><?php echo $webcast->title ?></option>
                        <?php endforeach; ?>
                    </optgroup>
                <?php endif; ?>
        ...
        <div class="dcf-form-group">
            ...
            <textarea id="v-additional-public-info" name="v_additional_public_info"><?php echo $v_location_additional_public_info; ?></textarea>
        </div>
    </template>
    <hr>
</section>
...
$page->addScript($base_frontend_url .'templates/default/html/js/manager-event-form-date-time.min.js?v='.UNL\UCBCN\Frontend\Controller::$version);
